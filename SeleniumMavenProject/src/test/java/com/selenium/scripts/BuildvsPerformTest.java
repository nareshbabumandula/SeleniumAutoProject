...
public class BuildvsPerformTest {
	...
	public void draganddropMethods() throws InterruptedException {
		boolean bFlag=false;
		try {
			/*build() method in Actions class is use to create chain of actions or operation you want to perform.
		    ...
		    */
			...
			Action seriesOfActions = builder.sendKeys(act, "Scru").keyUp(Keys.CONTROL).sendKeys(act, "M").keyDown(act, Keys.TAB).click().build();
			Thread.sleep(3000);
			seriesOfActions.perform();
			Thread.sleep(3000);
			String title = driver.getTitle();
	
			if(title.contains("Scrum")){
